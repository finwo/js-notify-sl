(function(factory) {
  ...
  function getStyle(property, element) {
    element = element || document.getElementById("notify-style") || (document.getElementsByClassName("card") || [])[0] || (document.getElementsByClassName("main") || [])[0] || (document.getElementsByClassName("form") || [])[0] || document.body;
  ...
  function append(element, contents) {
    if (Array.isArray(contents)) {
      return contents.map(append.bind(null, element));
    }
    if ("string" === typeof contents) {
      return element.innerHTML += contents;
    }
    if (contents instanceof Node) {
      return element.appendChild(contents);
    }
    return false;
  }
  function on(element, event, handler) {
    if (Array.isArray(event)) {
      return event.map(function(e) {
        return on(element, e, handler);
      });
    }
    if (Array.isArray(handler)) {
      return handler.map(on.bind(null, element, event));
    }
  ...
  notify.open = function(options) {
    ...
    if (options.contents) {
      ...
      if (options.style.titleColor)
        title.style.color = options.style.titleColor;
      message.style.marginBottom = 0;
      message.style.paddingBottom = 0;
      if (title.innerText)
        append(notification.element, title);
      if (message.innerText)
        append(notification.element, message);
    }
    if (options.buttons) {
  ...
  notify.prompt = function(message, title, data, callback) {
    ...
    if (elTitle.innerHTML)
      contents.push(elTitle);
    if (elMessage.innerHTML)
      contents.push(elMessage);
